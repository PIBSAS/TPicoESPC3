...
esp = ESPC3()  # Se puede activar el debug ingresando el parámetro debug=True
...
try:
    ...
    velocidad_scroll = 3  # Velocidad del scroll en píxeles
    retardo_scroll = 0.03  # Retardo entre desplazamientos para suavizar
    ...
    # Obtener el ancho máximo de texto
    max_desplazamiento = max(len(texto) * 8 for texto in textos)

    # Ahora hacer scroll para todas las líneas al mismo tiempo
    if max_desplazamiento > pantalla_ancho:
        for desplazamiento in range(0, max_desplazamiento - pantalla_ancho + 1, velocidad_scroll):
            for i, texto in enumerate(textos):
                # Solo borrar la parte de la línea que se va a redibujar
                tft.fill_rect(0, y_positions[i], pantalla_ancho, 16, st7789.BLACK)
                # Dibujar el texto desplazado
                tft.text(font, texto, -desplazamiento, y_positions[i], st7789.CYAN)

            time.sleep(retardo_scroll)  # Ajusta la velocidad del scroll
